...
.im {
  /* 容器设置为相对定位，作为视频的定位参考 */
  position: relative;
  width: 100%; /* 容器宽度（可根据需求调整，如固定值） */
  height: 400px
; /* 容器高度（100vh 表示占满整个视口高度） */
...
.im video {
  ...
  width: 100%;
  height: 100%;
  z-index: 1; /* 内容层级3（最高，覆盖视频和遮罩） */
  /* 保证视频比例不变的情况下填满容器（可能会裁剪边缘） */
  object-fit: cover;
}
/* 虚化遮罩基础样式 */
.mask {
  position: absolute;
  width: 100%;
  /* 遮罩高度（可调整，这里设为15%视口高度） */
  height: 15vh;
  pointer-events: none; /* 让遮罩不影响鼠标交互（如点击视频下方内容） */
}
...
.index-container {
  ...
  padding: 0;
  .headerr{
    z-index: 3; /* 内容层级3（最高，覆盖视频和遮罩） */
    text-align: center;
    width: 100%;
    height: 100px;
    background: linear-gradient(to bottom, rgb(231, 244, 216), transparent);
    .text{
      padding: inherit;
      font-size: 36px;
      color: #919191;
